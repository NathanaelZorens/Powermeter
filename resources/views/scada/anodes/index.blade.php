        ...
        <div class="mx-auto" id="mainTable">
            ...
            <table class="gap-2 border-2 border-white border-solid border-spacing-2 mx-auto">
                ...
                <tr class="border-2 h-2 even:bg-gray-900 odd:bg-gray-800 text-lg" id="tblRow">
                    ...
                    <td class="w-20 h-2 m-auto text-center">{{$anode['phase']}}</td>
                    ...
                    $idNode = $anode['id'];
                    $nameNode = $anode['name'];
                    $phaseNode = $anode['phase'];
                    $acolNode = $anode['acol_id'];
                    $parentNode = $anode['parent_id'];
                    $colorNode = $anode['color'];

                    $nodeData = array($idNode, $nameNode, $phaseNode ,$acolNode, $parentNode, $colorNode);
                    ?>


                    <td class="w-32 m-auto justify-center">
                        <div class="flex">
                            <button class="bg-green-700 hover:bg-green-300 m-2 px-2 text-sm flex rounded border-solid border border-white " onclick="showSideED('{{$idNode}}','{{$nameNode}}','{{$phaseNode}}','{{$acolNode}}','{{$parentNode}}','{{$colorNode}}')">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                            ...
                            <form action="/anodes/{{$anode->id}}" method="post">
                                @method('delete')
                                @csrf
                                <button class="bg-red-600 hover:bg-red-300 m-2 px-2 text-sm flex rounded border-solid border border-white" onclick="return confirmDelete()">
